...
public class Chained<K, V> extends AbstractHM<K, V> {
    ...
    public void rehash() {
        ...
        LinkedList<Entry<K, V>>[] newTable = new LinkedList[primeCapacity];

        for (int i = 0; i < primeCapacity; i++) {
            newTable[i] = new LinkedList<>();
        }

        for (LinkedList<Entry<K, V>> backArr : table) {
            for (Entry<K, V> e : backArr) {
                int index = Math.floorMod(hashFun.apply(e.getKey()), primeCapacity);
                newTable[index].add(new Entry<>(e.getKey(), e.getValue()));
            }
        }
    ...
    public @NotNull V get(@NotNull K key) throws KeyNotFoundE {
        ...
        LinkedList<Entry<K, V>> buck = table[idx];
        for (Entry<K, V> b : buck) {
    ...
    public void remove(@NotNull K key) throws KeyNotFoundE {
        ...
        int index = hash(key);
        LinkedList<Entry<K, V>> backArr = table[index];
        for (int i = 0; i < backArr.size(); i++) {
            Entry<K, V> e = backArr.get(i);
